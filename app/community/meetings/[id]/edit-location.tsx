...
export default function EditLocationScreen() {
  ...
  const updateMutation = useMutation({
    mutationFn: (newLocation: string) =>
      updateMeeting(id, { location: newLocation }),
    onSuccess: (data) => {
      if (data.success) {
        ...
        >
          <VStack className="flex-1 px-4 pt-4">
            <Text className="mb-8 text-2xl font-bold text-typography-900">
              {t("community.enterLocation", "모임 장소를 입력해주세요")}
            </Text>

            <VStack space="sm">
              <Text className="text-sm font-medium text-primary-500">
                {t("community.location", "장소")}
              </Text>
              <Input
                variant="underlined"
                size="lg"
              ...
              >
                <InputField
                  ...
                  placeholder={t("community.location", "장소")}
            ...
            <View className="flex-1 justify-end pb-8">
              <Button
                ...
                onPress={() => updateMutation.mutate(location)}
              >
